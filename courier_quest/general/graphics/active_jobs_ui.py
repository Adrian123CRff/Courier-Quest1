...
class ActiveJobsUI:
    ...
    def draw(self) -> None:
        ...
        # Medidas del panel horizontal en la parte superior (solo ancho del mapa)
        map_width = getattr(v, 'MAP_WIDTH', v.width)
        panel_w = max(200, map_width - 20)
        ...
        # Título (alineado a la izquierda, más compacto)
        arcade.Text("📦 PEDIDOS ACTIVOS", left + 12, top - 18, (255, 165, 0), 12, bold=True).draw()

        if v.job_manager and v.game_manager:
            try:
                active_jobs = [j for j in v.job_manager.all_jobs() if
                               getattr(j, "accepted", False) and not getattr(j, "completed", False)]
                ...
                if not active_jobs:
                    ...
                    arcade.Text("No hay pedidos activos", left + 16, top - 40, (180, 196, 220), 10).draw()
                    try:
                        available = v.job_manager.get_available_jobs(v.game_manager.get_game_time())
                        if available:
                            arcade.Text(f"{len(available)} pedidos disponibles", left + 16, top - 55, (144, 238, 144),
                                        9).draw()
                    except Exception:
                        pass
                else:
                    ...
                    item_spacing = 130
                    x_offset = 0
                    for i, job in enumerate(active_jobs[:8]):
                        x_pos = left + 16 + (x_offset * item_spacing)  # Espaciado horizontal compacto
                        ...
                        # Dibujar información del pedido horizontalmente (alineado izquierda y más pequeño)
                        arcade.Text(status_text, x_pos, top - 40, color, 10).draw()
                        ...
                        arcade.Text(release_time_text, x_pos, top - 54, release_color, 8).draw()
                        if deadline_text:
                            arcade.Text(deadline_text, x_pos, top - 66, deadline_color, 8).draw()

                        x_offset += 1

            except Exception as e:
                arcade.Text(f"Error: {str(e)[:30]}...", left + 16, top - 40, (255, 120, 120), 10).draw()
        else:
            arcade.Text("Sistemas cargando...", left + 16, top - 40, (180, 196, 220), 10).draw()
                    ...
                    if jid not in alerted:
                        if hasattr(view, "show_notification"):
                            view.show_notification(
                                f"⏰ Pedido {jid} por vencer: {minutes:02d}:{seconds:02d} ({percent:.0f}%)")
                        alerted.add(jid)
            except Exception:
                pass
            return txt, col
