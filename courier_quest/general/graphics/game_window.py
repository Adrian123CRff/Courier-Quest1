...
class MapPlayerView(View):
    def __init__(self, state) -> None:
        ...
        try: self.player.bind_stats(self.player_stats)
        except Exception: self.player.bound_stats = self.player_stats
        ...
        # RESUME flags
        self._resume_mode = bool((isinstance(self.state, dict) and self.state.get("__resume_from_save__")) or getattr(self.state, "__resume_from_save__", False))
    ...
    def _initialize_game_systems(self):
        try:
            ...
            try: self.game_manager.initialize_game(map_data, jobs_data, weather_data)
            except Exception: pass

            try:
                if self.game_manager: self.game_manager.set_game_map(self.game_map)
            except Exception: pass
            ...
            if self._resume_mode:
                ...
                try:
                    ...
                    if not ws:
                        ws = self.state.get("weather_data") if isinstance(self.state, dict) else getattr(self.state, "weather_data", {})
                ...
                # posición desde save (por si otro flujo no la fijó)
                try:
    ...
    def _fast_forward_elapsed(self):
        """Empuja el tiempo al elapsed guardado. Si no hay setters en GameManager,
        aplica un 'offset' envolviendo los getters."""
        try:
            elapsed = None
            if isinstance(self.state, dict):
            ...
            for attr in ("_elapsed", "elapsed", "time_elapsed", "game_time"):
                if hasattr(gm, attr):
                    try:
                        ...
                        # si el manager usa deltas desde _last_update, reinicialízalo
            ...
            try:
                ...
                if hasattr(gm, "get_current_map_time") and callable(gm.get_current_map_time):
                    import datetime
                    # Recalcula a partir de la hora de inicio si existe; si no, deja el original
    ...
    def set_game_systems(self, game_manager, job_manager):
        ...
        if game_manager:
            try: game_manager.player_manager = self.player
            except Exception: pass
        self._load_initial_jobs()

    def _load_initial_jobs(self):
        # 1) leer lista del save (compat)
        if isinstance(self.state, dict):
        ...
        # 3) sembrar los aceptados usando SUS coordenadas guardadas
        if self.job_manager:
            for raw in self.accepted_raw_jobs:
                try:
                    jid = raw.get("id") or raw.get("job_id")
                    # respetar pickup/dropoff del snapshot
                    ...
                    # muchos managers aceptan (raw, spawn_hint). Enviamos el pickup guardado si existe.
                    ...
                    if job:
                        # forzar campos críticos a lo guardado
                        if saved_pickup is not None:  job.pickup = saved_pickup
                        if saved_dropoff is not None:  job.dropoff = saved_dropoff
                        ...
                        # si estaba recogido en el guardado, que NO dibuje pickup; opcional: meter en inventario
                        if job.picked_up:
    ...
    def _spawn_next_notification_immediate(self):
        if not self.incoming_raw_jobs: return
    ...
    def _accept_notification(self):
        if not self.job_notification_data: return
        ...
        try:
            for job in self.job_manager.all_jobs():
                if getattr(job, "accepted", False) and not getattr(job, "picked_up", False):
                    ...
                    Text("PICKUP", px - 20, py + 15, arcade.color.BLACK, 8).draw()
                if getattr(job, "picked_up", False) and not getattr(job, "completed", False):
                    ...
                    arcade.draw_rect_filled(dx, dy, TILE_SIZE * 0.6, TILE_SIZE * 0.6, arcade.color.RED)
                    arcade.draw_rect_outline(dx, dy, TILE_SIZE * 0.6, arcade.color.BLACK, 2)
                    Text("DROPOFF", dx - 25, dy + 15, arcade.color.WHITE, 8).draw()
        except Exception as e:
            print(f"[ERROR] Dibujando marcadores: {e}")

    def _draw_job_notification(self):
        if not self.job_notification_active or not self.job_notification_data: return
        raw = self.job_notification_data
        ...
        panel_width = 400; panel_height = 250
        left = SCREEN_WIDTH - panel_width - 20; bottom = 100
        right = left + panel_width; top = bottom + panel_height
    ...
    def on_draw(self) -> None:
        ...
        try: self.weather_renderer.draw()
        except Exception: pass
    ...
    def _draw_panel(self):
        ...
        cond = ws.get("condition", "?"); intensity = ws.get("intensity", "?"); multiplier = ws.get("multiplier", 1.0)
        ...
        if inventory:
            weight = getattr(inventory, "current_weight", 0); max_weight = getattr(inventory, "max_weight", 10)
            items = []
            try:
        ...
        if self.job_manager and self.game_manager:
            try:
                ...
                for job in active_jobs[:4]:
                    ...
                    job_id = getattr(job, "id", "Unknown"); payout = getattr(job, "payout", 0)
                ...
                if not jobs_info:
                    ...
                    try: available = self.job_manager.get_available_jobs(self.game_manager.get_game_time())
        ...
        if self.score_system:
            try:
                ...
                tr = stats.get('time_remaining', 0); minutes = int(tr // 60); seconds = int(tr % 60)
        ...
        bar_w, bar_h = 200, 20; left = MAP_WIDTH + 50; bottom = 30
        right = left + bar_w; top = bottom + bar_h
    ...
    def _draw_time_panel(self):
        ...
        panel_x = 10; panel_y = SCREEN_HEIGHT - 100; panel_width = 300; panel_height = 90
        ...
        try:
            ...
            minutes = int(current_time // 60); seconds = int(current_time % 60)
            time_remaining = self.game_manager.get_time_remaining()
            rem_minutes = int(time_remaining // 60); rem_seconds = int(time_remaining % 60)
            current_map_time = self.game_manager.get_current_map_time()
            time_str = current_map_time.strftime("%H:%M:%S"); date_str = current_map_time.strftime("%Y-%m-%d")
    ...
    def on_update(self, dt: float) -> None:
        if self.game_manager:
            try: self.game_manager.update(dt)
            except Exception as e: print(f"Error en game_manager.update: {e}")

        if self.job_notification_active:
            self.job_notification_timer -= dt
            if self.job_notification_timer <= 0: self._reject_notification()

        if self.next_spawn_timer > 0.0: self.next_spawn_timer -= dt
        if not self.job_notification_active: self._maybe_start_notification()

        if self.active_notification and self.notification_timer > 0:
            self.notification_timer -= dt
            if self.notification_timer <= 0: self.active_notification = None

        inventory = self.state.get("inventory", None) if isinstance(self.state, dict) else getattr(self.state, "inventory", None)
        was_moving = bool(self.player.moving)
        try: self.player.update(dt, player_stats=self.player_stats, weather_system=self.weather_markov, inventory=inventory)
        except Exception:
        ...
        if was_moving and not self.player.moving:
            try:
                if self.game_manager and hasattr(self.game_manager, 'try_pickup_at'):
                    if self.game_manager.try_pickup_at(self.player.cell_x, self.player.cell_y):
                        self.show_notification("¡Paquete recogido! Ve al punto de entrega.")
            except Exception as e: print(f"Error pickup: {e}")
            try:
                if self.game_manager and hasattr(self.game_manager, 'try_deliver_at'):
                    ...
                    if result: self.show_notification(f"¡Pedido {result['job_id']} entregado!\n+${result['pay']:.0f}")
            except Exception as e: print(f"Error deliver: {e}")
        ...
        try:
            ...
            except Exception: current_weather = "clear"

            self.player_stats.update(
                dt, bool(self.player.moving), getattr(self, "at_rest_point", False),
                float(getattr(inventory, "current_weight", 0.0)) if inventory is not None else 0.0,
                current_weather, input_active=(time.time() - getattr(self, "_last_input_time", 0.0)) < 0.25
            )
        except Exception as e:
            print(f"Error actualizando player_stats: {e}")

        # clima (congelado si es reanudación)
        try:
            if self._freeze_weather:
                ws = self._resume_weather_state or (self.state.get("weather_state") if isinstance(self.state, dict) else getattr(self.state, "weather_state", {})) or {}
    ...
    def on_key_press(self, key: int, modifiers: int) -> None:
        ...
        if key == arcade.key.A:
            if self.job_notification_active and self.job_notification_data:
                self._accept_notification(); return
            if self._pending_offer:
                try:
                    on_accept, _ = self._pending_offer
                    if on_accept: on_accept(None)
                finally:
                    self._pending_offer = None; self._offer_job_id = None
                return
        if key == arcade.key.R:
            if self.job_notification_active and self.job_notification_data:
                self._reject_notification(); return
            if self._pending_offer:
                try:
                    _, on_reject = self._pending_offer
                    if on_reject: on_reject(None)
                finally:
                    self._pending_offer = None; self._offer_job_id = None
                return

        dx, dy = 0, 0
        if key == arcade.key.UP: dy = -1; self.facing = "up"
        elif key == arcade.key.DOWN: dy = 1; self.facing = "down"
        elif key == arcade.key.LEFT: dx = -1; self.facing = "left"
        elif key == arcade.key.RIGHT: dx = 1; self.facing = "right"
        else: return

        self._apply_facing()
        if self.game_manager and hasattr(self.game_manager, 'handle_Player_movement'):
            try: self.game_manager.handle_Player_movement(dx, dy); return
            except Exception: pass
    ...
    def on_key_release(self, key: int, modifiers: int):
        if not self._pending_offer: return
        try:
            on_accept, on_reject = self._pending_offer
            if key == arcade.key.A and on_accept: on_accept(None)
            elif key == arcade.key.R and on_reject: on_reject(None)
        finally:
            self._pending_offer = None; self._offer_job_id = None
    ...
    def _apply_facing(self):
        ...
        try: spr.scale = mag
            ...
            try: spr.scale_x = mag; spr.scale_y = mag
            except Exception: pass
